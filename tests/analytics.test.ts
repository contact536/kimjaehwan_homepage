...
import { countryFromIp } from '../server/country.ts';
...
test('anonymous page statistics count daily visitors, exclude admins and bots, and protect the report', async () => {
  ...
  try {
    ...
    const first = await recordVisit(db, visit('/index.html', '', 'Mozilla/5.0', 'https://search.example/query?q=private'), html(), 'KR', 'https://kimjaehwan.com');
    const cookie = first.headers.get('set-cookie')!.split(';')[0];
    assert.match(first.headers.get('set-cookie')!, /HttpOnly.*SameSite=Lax.*Secure/);
    await recordVisit(db, visit('/pages/research.html', cookie), html(), 'KR', 'https://kimjaehwan.com');
    await recordVisit(db, visit('/', cookie), html(), 'KR', 'https://kimjaehwan.com');
    await recordVisit(db, visit('/', '', 'Mozilla/5.0'), html(), 'US', 'https://kimjaehwan.com');
    await recordVisit(db, visit('/', '', 'Googlebot'), html(), 'KR', 'https://kimjaehwan.com');
    await recordVisit(db, visit('/', 'research_session=fake'), html(), 'KR', 'https://kimjaehwan.com');
    await recordVisit(db, visit('/admin/', 'research_session=fake'), html(), 'KR', 'https://kimjaehwan.com');
    ...
    assert.doesNotMatch(text, /203\.0\.113\.75|private|\/query/);
