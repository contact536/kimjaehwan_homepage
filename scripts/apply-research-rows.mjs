...
function row(r){const v=presentation[r.id];return `<article class="research-entry" data-research="${esc(r.id)}"><aside class="research-preview" aria-label="${esc(r.title)} 연구 분야"><span>${esc(v.category)}</span><strong>${esc(r.title)}</strong><ul>${v.concepts.map(c=>`<li>${esc(c)}</li>`).join('')}</ul></aside><div class="research-copy"><h3><a href="/pages/research.html#${esc(r.id)}">${esc(r.title)} <span>— ${esc(r.subtitle)}</span></a></h3><p class="research-meta">XAIKOREA <span aria-hidden="true">·</span> <span class="research-status">${esc(r.status)}</span></p>${v.role?`<p class="research-role">김재환 · ${esc(v.role)}</p>`:''}<nav class="research-resources" aria-label="${esc(r.title)} 자료 링크"><a href="/pages/research.html#${esc(r.id)}">연구 상세</a><span aria-hidden="true">/</span><a href="${v.related}">${v.label}</a></nav><p class="research-summary">${esc(v.summary)}</p><details class="research-expand"><summary>${esc(r.title)} 상세 설명</summary><p>${esc(r.description)}</p><p class="research-source">자료 기준: ${esc(r.source)}</p></details></div></article>`}
