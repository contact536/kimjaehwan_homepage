...
if (!tools.length) throw new Error('At least one internal tool is required.');
...
const cards = tools.map((tool, index) => {
  ...
  const attribution = tool.inventor ? `<p class="internal-tool-attribution"><span>발명자</span> <strong>${esc(tool.inventor)}</strong></p>` : '';
  return `<article class="internal-tool" id="internal-tool-${esc(tool.id)}" tabindex="-1"><header><img class="internal-tool-logo" src="${esc(tool.logo ?? `/assets/internal-tools/${tool.id}-logo-v1.webp`)}" alt="" width="256" height="256" loading="lazy" decoding="async"><div><p>${String(index + 1).padStart(2, '0')} · INTERNAL TOOL</p><h3>${esc(tool.name)} <small>${esc(tool.koreanName)}</small></h3></div></header><p class="internal-tool-category">${esc(tool.category)}</p><p class="internal-tool-summary">${esc(tool.summary)}</p>${attribution}<footer><p>${esc(tool.status)}</p><details class="internal-tool-screens"><summary aria-controls="${esc(tool.id)}-screens">화면 미리보기 <span>${tool.images.length}장</span></summary><div class="internal-tool-panel" id="${esc(tool.id)}-screens"><header><strong>${esc(tool.name)} 화면</strong><button type="button" class="internal-tool-close" aria-label="${esc(tool.name)} 화면 미리보기 닫기" hidden>닫기 ×</button></header><div class="internal-tool-gallery" aria-label="${esc(tool.name)} 화면">${[primary, ...additional].map(image).join('')}</div></div></details></footer></article>`;
}).join('');
const projectStart = '<!-- internal-tools:start -->';
const projectEnd = '<!-- internal-tools:end -->';
const projectSection = `${projectStart}<section class="internal-tools" id="internal-tools" aria-labelledby="internal-tools-title"><header><div><p class="kim-overline">XAIKOREA / INTERNAL AI TOOLS</p><h2 id="internal-tools-title">회사 내부 연구·운영 도구</h2><p>온프레미스 RAG, 서적 문서화, 인식 모델 학습과 GitHub 코드 분석을 위한 ${tools.length}가지 도구입니다. 모두 사내 전용이며 외부 접속 주소가 없습니다.</p></div><p class="internal-tools-count"><strong>${String(tools.length).padStart(2, '0')}</strong><span>internal tools</span></p></header><div class="internal-tool-list">${cards}</div><footer><p><span class="internal-tools-hover-hint">카드 아래 ‘화면 미리보기’에 마우스를 올리거나 클릭해 화면을 확인하세요.</span><span class="internal-tools-touch-hint">카드 아래 ‘화면 미리보기’를 눌러 화면을 확인하세요.</span></p></footer></section>${projectEnd}`;
...
projectHtml = projectHtml.replace('</head>', '<link rel="stylesheet" href="/css/internal-tools.css?v=20260926-4"></head>');
projectHtml = projectHtml.replace(/<script\b[^>]*src="\/js\/internal-tools\.js(?:\?[^"]*)?"[^>]*><\/script>/gu, '');
projectHtml = projectHtml.replace('</body>', '<script src="/js/internal-tools.js?v=20260926-4" defer></script></body>');
...
projectHtml = projectHtml.replace(/<meta name="description" content="[^"]*">/u, `<meta name="description" content="김재환의 AI 연구 프로젝트와 XAIKOREA의 내부 연구·운영 도구 ${tools.length}종, 운영 서비스 및 공개 프로젝트.">`);
